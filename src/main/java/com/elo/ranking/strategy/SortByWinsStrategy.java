...
@Component
public class SortByWinsStrategy implements RankingStrategy<List<PlayerScoreCard>> {
    ...
    private Comparator<PlayerScoreCard> getComparator() {
        return (StringUtils.hasText(order) && order.equalsIgnoreCase("desc")) ? Comparator.comparingInt(PlayerScoreCard::getWins).reversed() : Comparator.comparingInt(PlayerScoreCard::getWins);
    }
}
